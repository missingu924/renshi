...
<% 
	...
	// 对象实例 
	SystemDictObj domainInstance = new SystemDictObj(); 
	// 用户信息 
	...
	{ 
		domainInstance = (SystemDictObj) domainInstanceObj; 
	} 
...
<html> 
	<head> 
		...
		{	 
			// 做必要的检查 
			if(!checkNull("c_dict_code","<%=domainInstance.getPropertyCnName("c_dict_code")%>")) return false; 
			if(!checkNull("c_dict_name","<%=domainInstance.getPropertyCnName("c_dict_name")%>")) return false; 
	...
	<body> 
		<form name="addOrModifyForm" id="addOrModifyForm" action="<%=contextPath%>/Proxy/Servlet?servlet=<%=basePath%>&method=addOrModify4this" method="post"> 
			...
			<table width="700" align="center" class="detail_table detail_table-bordered"> 
				...
				<tr> 
					<td> 
						<%=domainInstance.getPropertyCnName("c_dict_code") %>: 
					</td> 
					<td> 
						<input name="c_dict_code" type="text" id="c_dict_code" value="<%=StringUtil.getNotEmptyStr(domainInstance.getC_dict_code(),"")%>" size="20"  <%=isModify?"readOnly":""%>> 
						<font color="red">*</font> 
				...
				<tr> 
					<td> 
						<%=domainInstance.getPropertyCnName("c_dict_name") %>: 
					</td> 
					<td> 
