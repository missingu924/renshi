...
<%@page import="com.wuyg.auth.searchcondition.AuthUserSearchCondition"%> 
<%@page import="com.wuyg.auth.obj.AuthUserObj"%> 
...
<% 
	...
	// 该功能对象查询条件实例 
	 com.wuyg.auth.searchcondition.AuthUserSearchCondition domainSearchCondition = (com.wuyg.auth.searchcondition.AuthUserSearchCondition) request.getAttribute("domainSearchCondition"); 
	// 用户信息 
	...
	<body> 
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/<%=basePath%>/Servlet?method=list4this"> 
			...
			<table class="search_table" align="center" width="98%"> 
				...
				<tr> 
					...
					<td> 
						...
						<input name="addButton" type="button" class="button button_add" value="增加" onClick="winOpen('<%=contextPath%>/<%=basePath%>/Servlet?method=preModify4this')"> 
						&nbsp;&nbsp; 
			...
			<table id="search_condition_table" class="search_condition_table" align="center" width="98%" style='display: <%=domainSearchCondition.isShowSearchConditionTable() ? "" : "none"%>'> 
							<input type="hidden" id="showSearchConditionTable" name="showSearchConditionTable" value="<%=domainSearchCondition.isShowSearchConditionTable()%>"> 
				<tr> 
						<td><%=domainInstance.getPropertyCnName("account") %></td> 
						<td>
						...
						<td>
						<%=DictionaryUtil.getInputHtml("是否字典", "enable", StringUtil.getNotEmptyStr(domainInstance.getEnable(), ""),20)%> 
						</td> 
				</tr> 
			</table> 
			...
			<table id="data_table" class="table table-bordered table-striped" align="center" width="98%"> 
				...
				<input type="hidden" name="orderBy" id="orderBy" value="<%=StringUtil.getNotEmptyStr(domainSearchCondition.getOrderBy(), "")%>"> 
					<tr> 
						<th onClick="sortBy(this)" db_col="account" class="<%=domainSearchCondition.getSortClassByDbColumn("account")%>"><%=domainInstance.getPropertyCnName("account") %></th> 
						...
						<th onClick="sortBy(this)" db_col="telephone" class="<%=domainSearchCondition.getSortClassByDbColumn("telephone")%>"><%=domainInstance.getPropertyCnName("telephone") %></th> 
						<th onClick="sortBy(this)" db_col="enable" class="<%=domainSearchCondition.getSortClassByDbColumn("enable")%>"><%=domainInstance.getPropertyCnName("enable") %></th> 
						<th>操作</th> 
					</tr> 
				</thead> 
				<% 
				...
				<tr> 
					...
					<td width="100" style="text-align:center"> 
						...
						&nbsp; 
						<input type="button" class="button button_modify" title="修改" onClick="winOpen('<%=contextPath%>/<%=basePath%>/Servlet?method=preModify4this&<%=o.findKeyColumnName()%>=<%=o.findKeyValue()%>')" /> 
						&nbsp; 
