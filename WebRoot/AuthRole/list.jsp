...
<%@page import="com.wuyg.auth.searchcondition.AuthRoleSearchCondition"%> 
...
<% 
	...
	// 该功能对象查询条件实例 
	 com.wuyg.auth.searchcondition.AuthRoleSearchCondition domainSearchCondition = (com.wuyg.auth.searchcondition.AuthRoleSearchCondition) request.getAttribute("domainSearchCondition"); 
	// 用户信息 
		...
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/<%=basePath%>/Servlet?method=list4this"> 
			...
			<table id="data_table" class="table table-bordered table-striped" align="center" width="98%"> 
				...
				<input type="hidden" name="orderBy" id="orderBy" value="<%=StringUtil.getNotEmptyStr(domainSearchCondition.getOrderBy(), "")%>"> 
					<tr> 
						<th onClick="sortBy(this)" db_col="rolecode" class="<%=domainSearchCondition.getSortClassByDbColumn("rolecode")%>"><%=domainInstance.getPropertyCnName("rolecode") %></th> 
						<th onClick="sortBy(this)" db_col="rolename" class="<%=domainSearchCondition.getSortClassByDbColumn("rolename")%>"><%=domainInstance.getPropertyCnName("rolename") %></th> 
						<th onClick="sortBy(this)" db_col="rolediscription" class="<%=domainSearchCondition.getSortClassByDbColumn("rolediscription")%>"><%=domainInstance.getPropertyCnName("rolediscription") %></th> 
						<th>操作</th> 
					</tr> 
				</thead> 
				<% 
