...
<%@page import="com.wuyg.auth.searchcondition.AuthOfficeSearchCondition"%> 
...
<% 
	...
	// 该功能对象查询条件实例 
	 com.wuyg.auth.searchcondition.AuthOfficeSearchCondition domainSearchCondition = (com.wuyg.auth.searchcondition.AuthOfficeSearchCondition) request.getAttribute("domainSearchCondition"); 
	// 用户信息 
		...
		<form name="pageForm" id="pageForm" method="post" action="<%=request.getContextPath()%>/<%=basePath%>/Servlet?method=list4this"> 
			...
			<table id="search_condition_table" class="search_condition_table" align="center" width="98%" style='display: <%=domainSearchCondition.isShowSearchConditionTable() ? "" : "none"%>'> 
				...
				<tr> 
						...
						<td><%=domainInstance.getPropertyCnName("officename") %></td> 
						<td>
			...
			<table id="data_table" class="table table-bordered table-striped" align="center" width="98%"> 
				...
				<input type="hidden" name="orderBy" id="orderBy" value="<%=StringUtil.getNotEmptyStr(domainSearchCondition.getOrderBy(), "")%>"> 
					<tr> 
						<th onClick="sortBy(this)" db_col="officecode" class="<%=domainSearchCondition.getSortClassByDbColumn("officecode")%>"><%=domainInstance.getPropertyCnName("officecode") %></th> 
						<th onClick="sortBy(this)" db_col="officename" class="<%=domainSearchCondition.getSortClassByDbColumn("officename")%>"><%=domainInstance.getPropertyCnName("officename") %></th> 
						<th onClick="sortBy(this)" db_col="officecomment" class="<%=domainSearchCondition.getSortClassByDbColumn("officecomment")%>"><%=domainInstance.getPropertyCnName("officecomment") %></th> 
						<th>操作</th> 
					</tr> 
				</thead> 
				<% 
