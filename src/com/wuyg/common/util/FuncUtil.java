	...
	public static String invokeCustomMethed(String line)
	{
		try
		{
			...
			for (int i = 0; i < funcList.size(); i++)
			{
				...
				if (m.matches())
				{
					log.info(")չ�������-ԭʼ����:" + function);
					String methodName = m.group(1);
					String param = m.group(2);

					List<String> paramList = StringUtil.getStringListByString(param, ',', '"');// �зֲ���
					String[] params = new String[paramList.size()];
					for (int j = 0; j < params.length; j++)
					{
						String paramWithQuotation = paramList.get(j);
						params[j] = paramList.get(j).substring(1, paramWithQuotation.length() - 1);// ȥ�����}�ߵ����
					}
